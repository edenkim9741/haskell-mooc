...
capitalize str = unwords . map firstUpper $ words str
...
joinToLength n strings = filter ((==n).length) [first ++ last | first <- strings, last <- strings]
...
multiCompose [] a = a
multiCompose xs a = multiCompose (init xs) (last xs a)
...
multiApp func xs a = func $ map ($ a) xs
