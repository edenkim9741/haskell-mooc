...
isZero :: Integer -> Bool
-- isZero num = if num == 0 then True else False
...
sumTo n = if n > 1 then sumTo (n-1) + n else 1
...
power n k = if k > 0 then power n (k-1) * n else 1
...
ilog3 n = if n >= 3 then ilog3 (div n 3) + 1 else 1
